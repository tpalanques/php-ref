...
class ref {
     ...
     *
     * @param array|null &$options Optional, options to gather (from operators)
     * @return  array             Array of string expressions
     */
    public static function getInputExpressions(array|null &$options = null) {
     ...
     * @return  string
     */
    protected function fromReflector($reflector, $single = '', \Reflector|null $context = null) {
        ...
        foreach ($items as $item) {
            ...
            $comments = $this->isInternal($item) ? [] : static::parseComment($this->getDocComment($item));
            $meta = ['sub' => []];
            $bubbles = [];
            if ($this->isInternal($item)) {
                ...
                $comments = static::parseComment($this->getDocComment($item));
                if ($comments)
                    $meta += $comments;
                $meta['sub'][] = ['Defined in', basename($this->getFileName($item)) . ':' . $this->getStartLine($item)];
            }
            if (($item instanceof \ReflectionFunction) || ($item instanceof \ReflectionMethod)) {
                ...
                if (($item instanceof \ReflectionMethod) && !$this->isInternal($item)) {
                    try {
            ...
            if (method_exists($item, 'isInterface')) {
    ...
    protected function evaluate(&$subject, $specialStr = false) {
        ...
        if ($props) {
            ...
            foreach ($props as $idx => $prop) {
                ...
                $meta = $this->isInternal($sourceClass) ? null : static::parseComment($this->getDocComment($prop));
                if ($meta) {
                    if ($inherited)
        ...
        if ($magicProps) {
            ...
            foreach ($magicProps as $name => $value) {
                ...
                try {
                    ...
                    $meta = static::parseComment($this->getDocComment($prop));
                } catch (\Exception $e) {
                    $meta = null;
                }
                $this->fmt->startRow();
        ...
        if ($methods && !$this->hasInstanceTimedOut()) {
            ...
            foreach ($methods as $idx => $method) {
                ...
                $paramCom = $this->isInternal($method) ? [] : static::parseComment($this->getDocComment($method), 'tags');
                ...
                foreach ($method->getParameters() as $idx => $parameter) {
                    ...
                    foreach ($paramCom as $tag) {
                        [$pcTypes, $pcName, $pcDescription] = $tag;
                        if ($pcName !== $paramName)
                            continue;
                    ...
                    if (!empty($paramClass)) {
                        ...
                        $this->fromReflector($paramClass, $paramClass->getName());
                        $this->fmt->endContain();
                        $this->fmt->sep(' ');
                    ...
                    if ($optional) {
                        try {
                            ...
                            if ($paramValue !== null) {
                                $this->fmt->sep(' = ');
                                if (static::$env['is546'] && !$this->isInternal($parameter->getDeclaringFunction()) && $parameter->isDefaultValueConstant()) {
    ...
    public function isInternal(
            ReflectionMethod|ReflectionNamedType|ReflectionFunctionAbstract|ReflectionObject|ReflectionClass $item): bool {
        if (!method_exists($item, 'isInternal')) {
            return false;
        }
    ...
    public function getDocComment(
            ReflectionMethod|ReflectionNamedType|ReflectionFunctionAbstract|ReflectionObject|ReflectionClass $item): string {
        if (!method_exists($item, 'getDocComment')) {
            return '';
        }
        return $item->getDocComment();
    }

    public function getFileName(
            ReflectionMethod|ReflectionNamedType|ReflectionFunctionAbstract|ReflectionObject|ReflectionClass $item): string {
        if (!method_exists($item, 'getFileName')) {
            return '';
        }
    ...
    public function getStartLine(
            ReflectionMethod|ReflectionNamedType|ReflectionFunctionAbstract|ReflectionObject|ReflectionClass $item): string {
        if (!method_exists($item, 'getStartLine')) {
            return '';
        }
